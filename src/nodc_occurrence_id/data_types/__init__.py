...
from nodc_occurrence_id import data_types
...
def get_database_path(name: str) -> pathlib.Path:
    if name not in data_types.get_database_names():
...
def get_occurrence_database_for_data_type(data_type: str) -> OccurrencesDatabase | None:
    cls = get_databases().get(get_database_name_for_data_type(data_type))
    if not cls:
        return None
    db_path = get_occurrence_database_path_for_data_type(data_type)
    return cls(db_path)
...
def get_databases() -> dict[str, Type[OccurrencesDatabase]]:
    ...
    for data_type, cls in utils.get_all_class_children(OccurrencesDatabase).items():
        dbs[get_database_name_for_data_type(data_type)] = cls
    return dbs
